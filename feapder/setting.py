...
KAFKA_FLUSH_TIMEOUT = int(os.getenv("KAFKA_FLUSH_TIMEOUT", 10))  # flush超时时间（秒）
...
"""
报警级别阈值，可选 DEBUG / INFO / WARNING / ERROR；优先级 >= 该阈值的告警才会发送
- DEBUG  : 仅供调试
- INFO   : 提示信息，需要关注
- WARNING: 工作时间处理
- ERROR  : 必须立即处理
默认 DEBUG 即全量发送，由使用者按需调整阈值
"""
WARNING_LEVEL = "DEBUG"
...
# 监控 measurement，默认由爬虫类名填充，可在 setting 中显式覆盖
INFLUXDB_MEASUREMENT = os.getenv("INFLUXDB_MEASUREMENT") or None
