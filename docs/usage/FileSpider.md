...
- **进度追踪**: 框架自动追踪每个任务的下载进度（成功数/失败数/跳过数/总数）
...
| `on_task_all_done(task, result, success_count, fail_count, skipped_count, total_count)` | 任务所有文件处理完毕的回调，在此 yield Item 或 update_task_batch 更新状态 |
