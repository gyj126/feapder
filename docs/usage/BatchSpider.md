...
注意：每个批次开始时，框架默认会将全部任务状态重置为0，然后重新抓取。-1表示任务在当前批次抓取失败，下一批次开始时会被重新置为待抓取
...
每个批次开始时，框架默认会将全部任务状态重置为0，然后重新抓取。但是有些需求是增量采集的，做过的任务无需再次处理。重置任务是`init_task`方法实现的，我们可以将`init_task`方法置空来实现增量采集
