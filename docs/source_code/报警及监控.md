...
框架内置了基于 **InfluxDB 2.x + Grafana** 的可视化监控能力：

- 配置好 `INFLUXDB_URL` / `INFLUXDB_TOKEN` / `INFLUXDB_ORG` / `INFLUXDB_BUCKET` 后，框架会自动把请求、入库、代理池、账号池等指标写入 InfluxDB
- 项目仓库下提供了 Grafana 看板模板 `dashboards/grafana/feapder.json`，导入即可使用，包含六大类视图：
  - 概览：总请求数 / 下载成功 / 下载失败 / 解析异常 / 成功率 / 入库总量
  - 下载监控：各状态请求速率、成功率趋势、失败 Top10
  - 数据入库：各表入库速率、字段命中统计 Top50
  - 代理池：代理事件速率、失效比例
  - 账号池：账号状态速率、账号 × 状态 Top50
  - 自定义指标：按 `metric × field` 的趋势

部署方式与导入步骤详见 [dashboards/grafana/README.md](https://github.com/Boris-code/feapder/tree/master/dashboards/grafana/README.md)，自定义打点请参考 [监控打点](source_code/监控打点)。