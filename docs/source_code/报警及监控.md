...
WARNING_SUCCESS_RATE = 0.8 # 请求成功率低于 WARNING_SUCCESS_RATE 则报警
...
当 pipeline 入库/更新累计失败次数超过 `EXPORT_DATA_MAX_FAILED_TIMES`（默认 10）时，可能触发 WARNING 级别告警。失败次数为进程内累计计数，导出成功后不重置。

- **告警出口**：由 `scheduler.check_task_status()` 统一发送，检测间隔 60 秒
- **告警名称**：使用爬虫**类名**（如 `RedditClean`），与失败请求数、成功率等其他监控告警一致
- **告警条件**：仅当失败次数**相对上次告警有新增**时才发送，消息展示**累计总次数**（如 247 → 248 时告警「失败次数：248」）
- **日志记录**：`item_buffer` 在每次 flush 失败时仍会即时写入错误日志（日志中显示 `redis_key`，便于与 Redis 表名对应），但不重复推送告警

同一轮失败连续增长时，`message_prefix`（如 `《RedditClean》爬虫导出数据失败`）仍受 `WARNING_INTERVAL`（3600 秒）频率限制，防止刷屏。


## 可视化监控

支持对爬虫运行情况进行监控，除了数据监控和请求监控外，用户还可自定义监控内容，详情参考[监控打点](source_code/监控打点.md)。

监控数据写入 InfluxDB 2.x，通过 Grafana 看板按 站点 -> 爬虫 -> 时间 维度可视化，只需配置数据源地址并导入看板 JSON 即可使用，详见 `monitoring/grafana/README.md`。
