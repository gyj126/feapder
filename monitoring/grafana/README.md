...
feapder 内置打点监控，运行时将请求下载、数据入库、运行时长等指标写入 InfluxDB 1.x，再通过 Grafana 看板可视化。整套监控只需配置数据源地址、导入看板 JSON 即可使用。
...
| `_classify` | 指标分组：`document`（下载）、`<表名>`（入库）、自定义分类 |
| `_key` | 序列名（折线名） |
...
导入后顶部有 站点(`project`) -> 爬虫(`spider`) -> 时间 的级联筛选；若 `INFLUXDB_MEASUREMENT` 改过名，需同步修改看板顶部 `measurement` 变量。
